            ...
            <el-scrollbar height=320 >
              <div class="fundiv-ch">
                ...
                <el-descriptions
                    ...
                    border
                    direction="vertical">
                  <el-descriptions-item label="坐标" :rowspan="3" >
                    <el-input type="textarea" v-model="source_value"  placeholder="原始坐标，使用 ','分割" style="margin-bottom: 10px"/>
                    <el-input type="textarea" v-model="target_value" disabled  placeholder="结果" />
                  </el-descriptions-item>
                  <el-descriptions-item label="原坐标" align="center">
                    <el-radio-group
                        v-model="original"
                        text-color="#626aef"
                        fill="rgb(239, 240, 253)"
                    >
                      <el-radio-button label="高德 gcj-02" value="gcj" />
                      <el-radio-button label="WGS84" value="wgc84" />
                      <el-radio-button label="横轴墨卡托" value="mct" />
                      <el-radio-button label="百度" value="bd" />
                    </el-radio-group>
                  </el-descriptions-item>
                  <el-descriptions-item label="目标坐标系" align="center">
                    <el-radio-group
                        v-model="target"
                        text-color="#626aef"
                        fill="rgb(239, 240, 253)"
                  ...
                  <el-descriptions-item label="操作" align="center">
                    <el-button type="primary" @click="transf">转换</el-button>
...
import {ref, onMounted} from 'vue'
    ...
    let currentWMS = ref('http://10.0.20.144:31080/geoserver/zsh/wms?layers=zsh:pipe_gcj&viewparams=tenantId:1231213132313213');
    // 矢量点图层的建立
    let format = new WKT();
    ...
    let clear_source = ()=>{
      source.clear();
      let center = [12958093.194423104,4827109.768191865]
     ...
     * 以下为坐标转换相关
     * */
    let source_value = ref('');
    let target_value = ref('');
    let original = ref(null);
    let target = ref(null);
    const yanzheng = ()=>{
      if(original.value ===null && target.value === null){
        ElMessage({
          message:'请选择转换的坐标系！',
          type:'error'
        });
    ...
    const transf = () =>{
      ...
      let methodName = original.value + "2" + target.value;

      console.log(Number(coord[0]))
      let res = transform[methodName](Number(coord[0]),Number(coord[1]))
      console.log(res)
      target_value.value = res[0]+","+res[1];
      ElMessage(
          {
            message:'转换成功!',
            type:'success'
