            ...
            <div class="bottom-card">
                <div style="width:100%;height:30%;display:flex;align-items:center;">
                    ...
                    <div style="width:40%;flex:1;display:flex;justify-content: flex-end;align-items:center;" @click="doLike">
                        ...
                        <span style="margin-left:2px">{{data.wantCount}}</span>
                    </div>
                </div>
                <div style="width:100%;height:50%;display:flex;align-items:center;">
                    <span class="card-title" style="font-size: 18px;width:60%">{{data.gameName}}</span>
...
<script setup>
    ...
    function doLike(){
        ...
        service.post('/game/want?userId=1848304672451002368&accId=1848304672451002369'
        // ,{
        //     // userId:currentUserId,
        //     // accId:data.value.id
        //     userId:'1848304672451002368',
        //     accId:'1848304672451002369'
        // }
        ).then(res=>{
            if(res.data.code === 200){
                if(res.data.data === true&&(!data.value.isLike)){
                    data.value.isLike = !data.value.isLike
                    ElMessage({
                        message: '已添加想要',
                        type: 'success',
                    })
                }else if(res.data.data === true&&data.value.isLike){
                    data.value.isLike = !data.value.isLike
                    ElMessage({
                        message: '已取消想要',
                        type: 'warning',
